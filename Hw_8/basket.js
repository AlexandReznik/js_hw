...
const basketClear = document.querySelector('.crearBasket')

basketClear.addEventListener('click', () => {
    document.querySelectorAll('.elmBasket').forEach(el => el.remove());
...
function renderProductInBasket(productId) {
    const basketRowEl = basketEl
        .querySelector(`.basketRow elmBasket[data-id="${productId}"]`);
    if (!basketRowEl) {
...
function renderNewProductInBasket(productId) {
    const productRow = `
      <div class="basketRow elmBasket" data-id="${productId}">
